...
def window_partition(x, window_size):
    b, h, w, c = x.shape
    # If h and w are not multiples of window_size, this will fail or produce wrong results.
    # We should ensure they are divisible or padded.
...
def window_reverse(windows, window_size, h, w):
    # Use -1 to let PyTorch infer the batch dimension, which works better with symbolic shapes.
    # This avoids the int() call on symbolic expressions.
...
class AttenBlocks(nn.Module):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, compress_ratio, squeeze_factor, conv_scale, overlap_ratio, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False):
        ...
        self.blocks = nn.ModuleList([
            HAB(dim=dim, input_resolution=input_resolution, num_heads=num_heads, window_size=window_size, shift_size=0 if (i % 2 == 0) else window_size // 2, compress_ratio=compress_ratio, squeeze_factor=squeeze_factor, conv_scale=conv_scale, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop=drop, attn_drop=attn_drop, drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path, norm_layer=norm_layer) for i in range(depth)
        ])
...
class HAT(nn.Module):
    ...
    def calculate_rpi_sa(self):
        ...
        # Use simple subtraction and scaling to avoid complex indexing logic in ONNX
    ...
    def calculate_rpi_oca(self):
        ...
        # Use unsqueeze for clearer broadcasting in ONNX
        relative_coords = coords_ext_flatten.unsqueeze(1) - coords_ori_flatten.unsqueeze(2)
        relative_coords = relative_coords.permute(1, 2, 0).contiguous()
